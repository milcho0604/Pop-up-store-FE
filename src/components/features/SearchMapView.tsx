...
export default function SearchMapView({ posts }: Props) {
  ...
  useEffect(() => {
    ...
    if (postsWithAddr.length === 0) return;
  ...
  const allGeocoded = geocodedCount >= postsWithAddr.length && postsWithAddr.length > 0;

  return (
    <div className="relative" style={{ height: 'calc(100dvh - 56px - 64px - 140px)' }}>
      <div ref={mapRef} className="w-full h-full rounded-2xl overflow-hidden bg-gray-100" />

      {/* 지오코딩 진행 표시 */}
      {!allGeocoded && postsWithAddr.length > 0 && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm px-4 py-1.5 rounded-full text-xs text-gray-500 shadow-sm pointer-events-none">
          위치 불러오는 중 {geocodedCount}/{postsWithAddr.length}
        </div>
      )}
