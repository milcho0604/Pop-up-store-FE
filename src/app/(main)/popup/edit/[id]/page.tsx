...
export default function PopupEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        setMemberEmail(post.memberEmail ?? '');
        setTitle(post.title ?? '');
        setContent(post.content ?? '');
        setStartDate(toDateInput(post.startDate));
        setEndDate(toDateInput(post.endDate));
        setCity(post.city ?? '');
        setDong(post.dong ?? '');
        ...
        if (post.postImgUrl) setPreviewUrl(post.postImgUrl);
      } catch {
        setError('팝업 정보를 불러오지 못했습니다.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/popup/${postId}`);
    } catch {
      setError('팝업 수정에 실패했습니다.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <>
      <div className="px-5 pt-4 pb-10">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-bold text-gray-900">팝업스토어 수정</h1>
          <button
            onClick={() => router.back()}
